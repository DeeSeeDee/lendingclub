...
module.exports = {
	getFilteredLoans: function(lookAtAllLoans, callback){
		var filteredLoansData = {};
		api.queryLoansRoute('listing', function(data){
			...
			var filteredNotes = data.loans.filter(function(loan){
				if(grades.indexOf(loan.subGrade) === -1){
					return false;
				}
			...
			filteredLoansData.loans = filteredNotes;
			filteredLoansData.grades = gradePicker;
			callback(filteredLoansData);
		}, lookAtAllLoans);
	}
};